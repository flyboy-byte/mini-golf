...
- Required env: `DATABASE_URL` (path to the SQLite database file, e.g. `file:./data/mini_golf.db` — the `file:` prefix is optional and stripped). The API server also requires `PORT` (binds to `127.0.0.1` by default; override with `HOST` if a service needs to listen on all interfaces) and `APP_PASSWORD` (shared HTTP Basic Auth password — any username is accepted, only the password is checked); optionally `CORS_ORIGIN` (comma-separated allowed origins; defaults to reflecting any origin if unset). The frontend requires `PORT` and `BASE_PATH`.
...
**Security middleware** (`src/app.ts`, `src/middlewares/`): `helmet()` for security headers, a global rate limiter (`rate-limit.ts`, 120 req/min/IP) on all of `/api`, and shared-password HTTP Basic Auth (`basic-auth.ts`) on every `/api` route except `/api/healthz` (must stay open for the systemd/monitoring health check). There is no per-user auth or session model — `APP_PASSWORD` gates the whole API, matching the "single shared password" pattern used by other apps on the same host. A catch-all error handler at the end of the middleware chain logs unhandled errors via pino and returns a generic 500 instead of leaking stack traces.
