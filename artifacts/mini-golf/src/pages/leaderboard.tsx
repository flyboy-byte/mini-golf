...
export default function Leaderboard({ params }: { params: { gameId: string } }) {
   ...
   if (gameLoading || lbLoading || !game || !leaderboard) return <div className="p-8 text-center text-muted-foreground font-medium">Loading leaderboard...</div>;
               ...
               <div key={entry.playerId} className="flex items-center justify-between p-4 bg-card border border-border rounded-2xl shadow-sm relative overflow-hidden group">
                  ...
                  <div className="flex items-center gap-5">
                     ...
                     <div>
                        ...
                        <p className="text-sm text-muted-foreground font-medium mt-0.5">{entry.holesCompleted} / {game.holes} holes</p>
                     </div>
                  </div>
         ...
         {!game.completedAt && (
            <div className="mt-10">
               ...
               <p className="text-center text-xs text-muted-foreground mt-4 font-medium">
                  Ending the game will lock all scores.
               </p>
            </div>
         )}
      </div>
