...
import { useCreateGame, useAddPlayer, GameInputPar } from '@workspace/api-client-react';
    ...
    try {
      const game = await createGame.mutateAsync({
        data: {
          name: name.trim(),
          holes: holes === 18 ? 18 : 9,
          par: par === 2 ? GameInputPar.NUMBER_2 : par === 4 ? GameInputPar.NUMBER_4 : GameInputPar.NUMBER_3,
        },
      });
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 pb-24 space-y-8">
      ...
      <div className="space-y-6">
        ...
        <div className="space-y-3">
          <label className="text-sm font-bold uppercase tracking-wider text-muted-foreground">Par per hole</label>
          <div className="flex gap-3">
            {[2, 3, 4].map((p) => (
              <button
                key={p}
                onClick={() => setPar(p)}
                className={`flex-1 py-4 rounded-2xl font-display font-bold text-lg border-2 transition-all ${par === p ? 'bg-primary text-primary-foreground border-primary' : 'bg-card border-border text-foreground hover:border-primary/50'}`}
              >
                Par {p}
              </button>
            ))}
